...
print('002 trainX6 valX6 test CREMA.  002 es igual que 001 pero haciendo drop 6 en conf matrix')
#train_df = pd.concat([CREMA_train_df, TESS_train_df, RAVDESS_train_df, SAVEE_train_df])#CREMA does not have surprise !!!!!!!!!
train_df = pd.concat([SAVEE_train_df, RAVDESS_train_df, TESS_train_df, EMOFILM_train_df, CAFE_train_df])#CREMA does not have surprise !!!!!!!!!
...
for c in checkpoints:
  ...
  for round in range(rondas):
      ...
      print('LR es: ',lr)
      print('WD es: ',wd)
      EXPERIMENT_FOLDER=DUMP_FOLDER+'experim_002_.'+tms+'.'+str(job_id)+'.'+TYPE_EXPERIMENT+'/'
      ...
      val_acc, val_loss, accuracy, test_loss, record, tmp_name, model = loop_with_att_mask(checkpoint, train_dataset, test_test_dataset, test_val_dataset, DATASET_n,
                                                 ...
                                                 N_EPOCHS = 20,epoch_ini=epoch_ini,best_acc=best_acc,freeze=False, testlabel='CREMA_ijns')
      ...
      display_confusion_matrix(df_record)#save plot files
      df_record2 = df_record[df_record['Model Prediction'] != 6]
      display_confusion_matrix(df_record2,secondName='Drop6_')
      ...
      display_confusion_matrix(df_record,str(round)+'_TestCR_',id2label)#save plot files
      display_confusion_matrix(df_record2,str(round)+'_TestCR_Drop6_',id2label)
