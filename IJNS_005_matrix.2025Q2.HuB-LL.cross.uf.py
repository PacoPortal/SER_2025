...
print('005 es cross.  trainX6 valX6 testX6')
train_df = pd.concat([CAFE_train_df, CREMA_train_df, EMOFILM_train_df, TESS_train_df, RAVDESS_train_df, SAVEE_train_df])#CREMA does not have surprise !!!!!!!!!

cat_df, cav_df = data_split(CAFE_test_df, frac=0.5)
crt_df, crv_df = data_split(CREMA_test_df, frac=0.5)
...
test_df = pd.concat([cat_df, crt_df, et_df, tt_df, rt_df, rt_df])
test_df.reset_index(inplace=True,drop=True)
val_df = pd.concat([cav_df, crv_df, ev_df, tv_df, rv_df, sv_df])
val_df.reset_index(inplace=True,drop=True)

val_df, test_df = data_split(CREMA_test_df,  frac=0.5)
...
EXPERIMENT_FOLDER=DUMP_FOLDER+'experim_005_.'+tms+'.'+str(job_id)+'.'+TYPE_EXPERIMENT+'/'
...
for c in checkpoints:
  ...
  for round in range(rondas):
      ...
      val_acc, val_loss, accuracy, test_loss, record, tmp_name, model = loop_with_att_mask(checkpoint, train_dataset, test_test_dataset, test_val_dataset, DATASET_n,
                                                 label2id, id2label, num_labels,lr, wd, tms, DUMP_FOLDER,
                                                 N_EPOCHS = 20,epoch_ini=epoch_ini,best_acc=best_acc,freeze=False, testlabel='CROSS_ijns')

      os.chdir(DUMP_FOLDER)
      plot_loss_acc(val_loss, val_acc, str(round))#save plot files
      
      RECORD = tmp_name + '.record.p'
      pickle.dump(record, open(RECORD, 'wb'))

      test_df = test_df # pickle.load(open(TEST_DF, 'rb'))

      df_record = incorrect_ones(record,test_df)
      display_confusion_matrix(df_record,str(round)+'_Cross_',id2label)#save plot files

      df_record2 = df_record[df_record['Model Prediction'] != 6]
      df_record2 = df_record2[df_record2['Ground Truth'] != 6]
      display_confusion_matrix(df_record2,str(round)+'_Cross_Drop6_',id2label)
