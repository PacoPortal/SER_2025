...
EXPERIMENT_FOLDER=DUMP_FOLDER+'experim_003_.'+tms+'.'+str(job_id)+'.'+TYPE_EXPERIMENT+'/'
...
for c in checkpoints:
  ...
  for round in range(rondas):
      ...
      val_acc, val_loss, accuracy, test_loss, record, tmp_name, model = loop_with_att_mask(checkpoint, train_dataset, test_test_dataset, test_val_dataset, DATASET_n,
                                                 ...
                                                 N_EPOCHS = 20,epoch_ini=epoch_ini,best_acc=best_acc,freeze=False, testlabel='RAV_y_CREMA_ijns')
      ...
      display_confusion_matrix(df_record,str(round)+'_TestRV_',id2label)#save plot files

      df_record2 = df_record[df_record['Model Prediction'] != 6]
      df_record2 = df_record2[df_record2['Ground Truth'] != 6]
      display_confusion_matrix(df_record2,str(round)+'_TestRV_Drop6_',id2label)

      feature_extractor = AutoFeatureExtractor.from_pretrained('facebook/hubert-large-ll60k', return_attention_mask=True)
      test_loader = get_data_loader(CREMA_df, feature_extractor, train_bs=1 )
      accuracy, test_loss, record = testing_att_mask(test_loader, model, verbose=False)
      
      os.chdir(DUMP_FOLDER)
      RECORD = tmp_name + '.record2.p'
      pickle.dump(record, open(RECORD, 'wb'))

      df_record = incorrect_ones(record,CREMA_df)
      display_confusion_matrix(df_record,'TestCR_',id2label)
