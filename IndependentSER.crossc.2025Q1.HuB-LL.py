...
#¡¡¡¡¡¡ GUARDAR Y CARGAR SIEMPRE LOS MISMOS SPLITS !!!!!!!!
...
test_df = pd.concat([ct_df, tt_df, rt_df, st_df])#CREMA does not have surprise !!!!!!!!!
test_df.reset_index(inplace=True,drop=True)
val_df = pd.concat([cv_df, tv_df, rv_df, sv_df])#CREMA does not have surprise !!!!!!!!!
...
for c in checkpoints:
  ...
  for round in range(rondas):
      ...
      val_acc, val_loss, accuracy, test_loss = loop_with_att_mask(checkpoint, train_dataset, test_test_dataset, test_val_dataset, DATASET_n,
                                                  ...
                                                  N_EPOCHS = 20,epoch_ini=epoch_ini,best_acc=best_acc,freeze=False)
      acc+=accuracy
