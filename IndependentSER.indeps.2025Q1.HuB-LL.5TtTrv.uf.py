...
train_df = pd.concat([SAVEE_train_df, CREMA_train_df, TESS_train_df, EMOFILM_train_df, CAFE_train_df])#CREMA does not have surprise !!!!!!!!!
train_df.reset_index(inplace=True,drop=True)
#data_distribution(train_df) plot of distributions
val_df = pd.concat([SAVEE_test_df, CREMA_test_df, TESS_test_df, EMOFILM_test_df, CAFE_test_df])
test_df =  RAVDESS_df#CREMA_train_df #pd.concat([RAVDESS_test_df, SAVEE_test_df])#CREMA does not have surprise !!!!!!!!!
...
for c in checkpoints:
  ...
  for round in range(rondas):
      ...
      #                                            N_EPOCHS = 20,epoch_ini=epoch_ini,best_acc=best_acc,freeze=False, testlabel='RAVDESS TtT')
