...
DATASET_n ="CAFE_CREMA_EMOFILM_RAVDESS_TESS "
#TYPE_EXPERIMENT = 'cross'
TYPE_EXPERIMENT = 'indep'
print('Datastet: ',DATASET_n,', y tipo de experimento: ',TYPE_EXPERIMENT,', test on SAVEE')
...
train_df = pd.concat([CAFE_df, CREMA_df, EMOFILM_df, RAVDESS_df,TESS_df])#CREMA does not have surprise !!!!!!!!!
train_df.reset_index(inplace=True,drop=True)
#data_distribution(train_df) plot of distributions
val_df = SAVEE_df#CREMA_test_df
test_df =  SAVEE_df#CREMA_train_df #pd.concat([RAVDESS_test_df, SAVEE_test_df])#CREMA does not have surprise !!!!!!!!!
...
for c in checkpoints:
  ...
  for round in range(rondas):
      ...
      val_acc, val_loss, accuracy, test_loss = loop_with_att_mask(checkpoint, train_dataset, test_test_dataset, test_val_dataset, DATASET_n,
                                                  ...
                                                  N_EPOCHS = 20,epoch_ini=epoch_ini,best_acc=best_acc,freeze=False, testlabel='SAVEE')
